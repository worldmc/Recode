...
class Auth
{
    ...
    /// Maybe be a bit more nice to lowlevel admins. now they just shut their doors and redirect them to login. (kinda harsh if you make a mistake yourself)!!!!
     ...
     */
    public static function checkAdminLVL1()
    {
        // initialize the session (if not initialized yet)
        Session::init();
        ...
        if (!Session::userIsLoggedIn() || Session::get("user_account_type") != 7 || Session::get("user_account_type") != 10) {
            // ... then treat user as "not logged in", destroy session, redirect to login page
            Session::destroy();
    ...
    public static function checkAdminLVL2()
    {
        // initialize the session (if not initialized yet)
        Session::init();
        ...
        // if user is not logged in or is not an admin (= not role type 7)
        if (!Session::userIsLoggedIn() || Session::get("user_account_type") != 10) {
            // ... then treat user as "not logged in", destroy session, redirect to login page
            Session::destroy();
            header('location: ' . Config::get('URL') . 'login');
            // to prevent fetching views via cURL (which "ignores" the header-redirect above) we leave the application
    ...
    public static function checkBeamerAuth()
    {
        // initialize the session (if not initialized yet)
        Session::init();
        ...
        // if user is not logged in or is not an admin (= not role type 5)
